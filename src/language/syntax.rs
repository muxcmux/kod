...
use std::{
    fmt::Write,
    borrow::Cow, cell::RefCell, collections::VecDeque, hash::{Hash, Hasher}, iter::Peekable, mem, ops, path::Path, sync::{atomic::{AtomicUsize, Ordering}, Arc}
};
...
use crate::{graphemes::grapheme_is_line_ending, history::Transaction, language::LANG_CONFIG, rope::RopeCursor, ui::theme::THEME};
                ...
                for mat in matches {
                        ...
                        (injection_capture, content_node)
                    {
                        if let Some(config) = (injection_callback)(&injection_capture) {
                            ...
                            log::error!("No injection lang found for: {:?}. Cancelling TS to avoid infinite loop", injection_capture);
